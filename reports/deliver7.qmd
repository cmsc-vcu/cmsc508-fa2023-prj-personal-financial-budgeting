---
title: Deliverable 7 - Database Implementation
date: 12/11/2023
...
* URL to your design video (Task 12): 
    - <https://vcu.zoom.us/rec/share/oCleWmpGURaTP66RLhkGcvkRuolHAeBSUXy7Iw0Mw0ZqyXja9pi2Ifq444VIk0V0.z9Rcgx_cklmfOsti>
   
* URL to your GITHUB repository: 
    - <https://github.com/cmsc-vcu/cmsc508-fa2023-prj-personal-financial-budgeting/tree/main>
...
1. **Query 1:** Retrieve User's Budgets
   - `π budget (σ userID = 'U1', User)`

2. **Query 2:** List All Transactions for a Specific User
   - `π transaction (σ userID = 'U3', User)`

3. **Query 3:** Calculate Total Price/Gain for a Budget
   - `∑ price/gain (σ BudgetID = 'B2', Budget)`

4. **Query 4:** Compare Budgeted vs. Actual Expenses for a Specific Budget
   - `π budgetID, Source, Price/Gain, Actual (σ BudgetID = 'B4', Budget)`

5. **Query 5:** Find Transactions Above a Certain Amount for a User
   - `π transaction, price/gain (σ userID = 'U2' ∧ price/gain > 100, User ⨝ Transaction)`

6. **Query 6:** Retrieve Budgets with Specific Time Periods
   - `π BudgetID, TimePeriod (σ TimePeriod = '2 months', Budget)`

7. **Query 7:** Determine Total Income for a User in a Year
   - `∑ price/gain (σ userID = 'U4' ∧ EXTRACT(Year FROM Date) = 2023, Transaction)`

8. **Query 8:** Find Largest Expense Category in a Budget
...
9. **Query 9:** Retrieve Transactions from a Specific Date for a User
   - `π transaction, Date (σ userID = 'U5' ∧ Date = '2023-10-17', User ⨝ Transaction)`

10. **Query 10:** List Transactions Categorized as 'Entertainment' for a User
    - `π transaction, source (σ userID = 'U1' ∧ source = 'Entertainment', User ⨝ Transaction)`

11. **Query 11:** Calculate Average Monthly Income for a User
    - `AVG(price/gain) (σ userID = 'U3' ∧ EXTRACT(Month FROM Date) = 10, Transaction)`

12. **Query 12:** Retrieve Transactions Below a Certain Amount in a Budget
    - `π transaction, price/gain (σ BudgetID = 'B6' ∧ price/gain < 200, Budget ⨝ Transaction)`

13. **Query 13:** Find Budgets Exceeded by Actual Expenses for a User
    - `π BudgetID, Source (σ userID = 'U2' ∧ Price/Gain < Actual, Budget)`

14. **Query 14:** Retrieve Financial Advisor's Clients
    - `π name (σ advisorID IS NOT NULL, User)`

15. **Query 15:** Calculate Total Expenses by Transaction Category for a User
...
16. **Query 16:** Retrieve All Transactions from a Specific Bank for a User
    - `π transaction, source (σ userID = 'U4' ∧ source = 'Bank', User ⨝ Transaction)`

17. **Query 17:** List Transactions Made on a Specific Date for a User
    - `π transaction, Date (σ userID = 'U1' ∧ Date = '2023-10-03', User ⨝ Transaction)`

18. **Query 18:** Retrieve Monthly Expenses in Excess of Budget for a User
    - `π budgetID, Price/Gain, Actual (σ userID = 'U2' ∧ EXTRACT(Month FROM Date) = 10, Budget ⨝ Transaction)`

19. **Query 19:** Calculate Average Monthly Expense for a User
    - `AVG(price/gain) (σ userID = 'U5' ∧ EXTRACT(Month FROM Date) = 10, Transaction)`

20. **Query 20:** Retrieve Transaction History from a Specific Bank for a User
...
Link to DDL code:
- <https://github.com/cmsc-vcu/cmsc508-fa2023-prj-personal-financial-budgeting/blob/main/reports/deliver7-ddl.sql>
...
Summarizing the database design solution, discussion of any design tradeoffs, and reflections on the overall process.

* URL link to Design Video 
    - <https://vcu.zoom.us/rec/share/oCleWmpGURaTP66RLhkGcvkRuolHAeBSUXy7Iw0Mw0ZqyXja9pi2Ifq444VIk0V0.z9Rcgx_cklmfOsti>
...
## 14) Demonstration of the API in action
...
* <https://github.com/cmsc-vcu/cmsc508-fa2023-prj-personal-financial-budgeting/tree/main/api>
...
### 11. Extended API Functionality
Develop additional API endpoints for more features and third-party integrations to have a live service.

### 12. User Feedback and Continuous Improvement
Implement a system for collecting user feedback for ongoing application improvement to allow for error handling that we may not be able to catch ourselves.

## 17) Reflections on the overall project

The database project went fairly well! We were able to achieve our primary objectives. We managed to fully build an user-friendly database that allows users to manage their finances. The core functionalities including tracking income and expenses, budgeting, along with financial reporting were put into our project as intended. Our current project can be attributed to a well-organized project plan and brainstorming at the beginning of the semester, effective team collaboration and having a clear path to what the database needed. 

Like any project, we faced challenges and learning curves along the way. One of the key things that allowed us to succeed in this project is having professor Leonard show us key concepts and how to implement them with Quarto Markdown. Having deadlines throughout the entire semester made this project come to a finished product in a smooth manner.

In conclusion, while there were some deviations from our initial plan, the project was largely successful in achieving its main goals. The experience from this project, along with the course has left the group with not only a functional product but also with key learnings that will help us in the future whenever we need database knowledge and implementation.j